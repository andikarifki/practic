...
const props = defineProps({
    tempats: Object, // Pagination object
    filters: Object,
});

const search = ref(props.filters?.search || "");

// Watcher untuk fitur pencarian (Search)
watch(
    search,
    debounce((value) => {
        router.get(
...
const hapusTempat = (id) => {
    if (confirm("Apakah Anda yakin ingin menghapus data tempat berobat ini?")) {
        // Menggunakan object parameter agar Ziggy tidak error
        router.delete(route("tempat.destroy", { tempatBerobat: id }));
    }
};
</script>
                ...
                >
                    <div class="p-6 overflow-x-auto">
                        <table class="min-w-full divide-y divide-emerald-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                <tr
                                    v-for="(tempat, index) in tempats.data"
                                    :key="tempat.id"
                                ...
                                <tr v-if="tempats.data.length === 0">
                                    <td
                                        colspan="4"
                                        class="px-6 py-10 text-center text-gray-400 italic"
                                    >
                                        Data tempat berobat tidak ditemukan.
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    >
                        <div class="text-sm text-gray-600">
                            Menampilkan {{ tempats.from || 0 }} sampai
                            {{ tempats.to || 0 }} dari {{ tempats.total }} data
                        </div>
                        <div class="flex flex-wrap gap-1">
                            <template
                                v-for="(link, k) in tempats.links"
                                :key="k"
                            >
                                <div
                                    v-if="link.url === null"
